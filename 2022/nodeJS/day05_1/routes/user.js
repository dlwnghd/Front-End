...
router.post("/", async (req, res, next) => {
    try {
        ...
        // 만약 해당 데이터가 있다면 사용 중인 메일
        if (emailCheck) {
            return res
                .status(403)
                .send("이미 사용 중인 이메일입니다");
        }
        // 없다면 사용자가 보낸 암호인 req.body.paswword를 bcrypt를 사용하여 단방향 암호화를 진행
        const hashedPassword = await bcrypt.hash(req.body.password, 11);

        // insert 합니다 User 테이블에 사용자가 보낸 req.body에 실린 데이터를 단, 여기서 email, name, password는
        // 키 값이므로 models의 user.js에 정의해놓은 컬럼명(필드명)과 같아야 한다
...
// 로그인 로그인하지 않은 사용자인지 검사하고 다음 모듈을 실행해라 = isNotLoggedin 로그인한 사용자인지 검사하고 다음 모듈을
// 실행해라 = isLoggedin
...
router.post("/login", isNotLoggedin, (req, res, next) => {
    ...
    passport.authenticate("local", (err, user, desc) => {
        if (err) {
            console.log(err);
            return next(err);
            // next는요 '다음' 모듈을 실행해라 next() next(err) 안에 내용을 담으면 해당 모듈로 '이동'해라
        }
        if (desc) {
            return res
                .status(401)
...
router.post("/logout", isLoggedin, (req, res) => {
    req.logout(() => {
        ...
        res.send("ok");
        // ok라는 메세지를 보내고 다시 hoem("/")으로 주소를 이동해라 res.redirect("/");
    });
});
